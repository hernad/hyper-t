...
export function bundle(entryPoints: IEntryPoint[], config: ILoaderConfig, callback: (err: any, result: IBundleResult | null) => void): void {
	...
	const code = require('fs').readFileSync(path.join(__dirname, '../../src/loader.js'));
	...
	if (!config.paths['base/nls']) {
		config.paths['base/nls'] = 'out-build/base/nls.build';
	}
	if (!config.paths['css']) {
		config.paths['css'] = 'out-build/css.build';
	}
	...
	loader(Object.keys(allMentionedModulesMap), () => {
		...
		const cssInlinedResources = loader('css').getInlinedResources();
		callback(null, {
