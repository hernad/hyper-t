...
var editorEntryPoints = [
	{
		...
		include: [],
		exclude: ['css', 'base/nls'],
		prepend: ['out-editor-build/css.js', 'out-editor-build/base/nls.js'],
	},
	{
...
var editorResources = [
	'out-build/{base,editor}/**/*.{svg,png}',
	'!out-build/base/browser/ui/splitview/**/*',
	'!out-build/base/browser/ui/toolbar/**/*',
	'!out-build/base/browser/ui/octiconLabel/**/*',
	'!out-build/workbench/**',
	'!**/test/**'
];
...
gulp.task('extract-editor-src', ['clean-editor-src'], function () {
	...
	standalone.extractEditor({
		...
		redirects: {
			'base/browser/ui/octiconLabel/octiconLabel': 'base/browser/ui/octiconLabel/octiconLabel.mock',
		},
...
gulp.task('optimize-editor', ['clean-optimized-editor', 'compile-editor-build'], common.optimizeTask({
	...
	loaderConfig: {
		paths: {
			...
			'css': 'out-editor-build/css.build',
			'base/nls': 'out-editor-build/base/nls.build',
			'vscode': 'empty:'
		}
	},
	bundleLoader: false,
...
gulp.task('extract-editor-esm', ['clean-editor-esm', 'clean-editor-distro', 'extract-editor-src'], function () {
	standalone.createESMSourcesAndResources2({
		...
		ignores: [
			...
			'css.build.js',
			'css.d.ts',
			'base/worker/workerMain.ts',
		],
		renames: {
			'base/nls.mock.ts': 'base/nls.ts'
		}
	});
});
...
gulp.task('editor-distro', ['clean-editor-distro', 'compile-editor-esm', 'minify-editor', 'optimize-editor'], function () {
	return es.merge(
		...
		gulp.src('src/monaco.d.ts')
			...
			.pipe(gulp.dest('out-monaco-editor-core/esm/editor')),
			...
			child.stdout.on('data', data => {
				...
				if (str.indexOf('Starting compilation') >= 0 || str.indexOf('File change detected') >= 0) {
					...
					if (match) {
						...
						// e.g. src/base/common/strings.ts(663,5): error TS2322: Type '1234' is not assignable to type 'string'.
