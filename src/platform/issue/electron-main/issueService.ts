...
import { parseArgs } from 'platform/environment/node/argv';
import { IIssueService, IssueReporterData, IssueReporterFeatures, ProcessExplorerData } from 'platform/issue/common/issue';
import { BrowserWindow, ipcMain, screen, Event } from 'electron';
import { ILaunchService } from 'platform/launch/electron-main/launchService';
import { PerformanceInfo, SystemInfo, IDiagnosticsService } from 'platform/diagnostics/electron-main/diagnosticsService';
import { IEnvironmentService } from 'platform/environment/common/environment';
import { isMacintosh, IProcessEnvironment } from 'base/common/platform';
import { ILogService } from 'platform/log/common/log';
import { IWindowsService } from 'platform/windows/common/windows';
import { IWindowState } from 'platform/windows/electron-main/windows';
...
export class IssueService implements IIssueService {
	...
	openProcessExplorer(data: ProcessExplorerData): Promise<void> {
		return new Promise(_ => {
			...
			if (!this._processExplorerWindow) {
				...
				this._processExplorerWindow.loadURL(`${require.toUrl('code/electron-browser/processExplorer/processExplorer.html')}?config=${encodeURIComponent(JSON.stringify(config))}`);
	...
	private getIssueReporterPath(data: IssueReporterData, features: IssueReporterFeatures): string {
		...
		return `${require.toUrl('code/electron-browser/issue/issueReporter.html')}?config=${encodeURIComponent(JSON.stringify(config))}`;
	}
}
