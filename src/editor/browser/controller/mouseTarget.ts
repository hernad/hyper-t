...
import { IMouseTarget, MouseTargetType } from 'editor/browser/editorBrowser';
import { ClientCoordinates, EditorMouseEvent, EditorPagePosition, PageCoordinates } from 'editor/browser/editorDom';
import { PartFingerprint, PartFingerprints } from 'editor/browser/view/viewPart';
import { ViewLine } from 'editor/browser/viewParts/lines/viewLine';
import { IViewCursorRenderData } from 'editor/browser/viewParts/viewCursors/viewCursor';
import { EditorLayoutInfo } from 'editor/common/config/editorOptions';
import { Position } from 'editor/common/core/position';
import { Range as EditorRange } from 'editor/common/core/range';
import { HorizontalRange } from 'editor/common/view/renderingContext';
...
class ElementPath {
	...
	public static isChildOfMinimap(path: Uint8Array): boolean {
		return (
			path.length >= 2
			&& path[0] === PartFingerprint.OverflowGuard
			// && path[1] === PartFingerprint.Minimap ; hernad minimap-out
		);
	}
	*/
...
export class MouseTargetFactory {
	...
	private static _createMouseTarget(ctx: HitTestContext, request: HitTestRequest, domHitTestExecuted: boolean): MouseTarget {
		...
		// result = result || MouseTargetFactory._hitTestMinimap(ctx, resolvedRequest);  ; hernad minimap-out
