...
import { IContextViewProvider, IAnchor } from 'base/browser/ui/contextview/contextview';
	...
	constructor(container: HTMLElement, options: IBaseDropdownOptions) {
		...
		[EventType.MOUSE_DOWN, GestureEventType.Tap].forEach(event => {
			this._register(addDisposableListener(this._label, event, e => {
				if (e instanceof MouseEvent && e.detail > 1) {
					return; // prevent multiple clicks to open multiple context menus (https://github.com/hernad/hyper-t/issues/41363)
				}
		...
		this._register(addDisposableListener(this._label, EventType.KEY_UP, e => {
			...
			if (event.equals(KeyCode.Enter) || event.equals(KeyCode.Space)) {
				EventHelper.stop(e, true); // https://github.com/hernad/hyper-t/issues/57997
