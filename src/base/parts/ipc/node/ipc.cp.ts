...
import { Delayer, always, createCancelablePromise } from 'base/common/async';
import { deepClone, assign } from 'base/common/objects';
import { Emitter, fromNodeEventEmitter, Event } from 'base/common/event';
import { createQueuedSender } from 'base/node/processes';
import { ChannelServer as IPCServer, ChannelClient as IPCClient, IChannelClient, IChannel } from 'base/parts/ipc/node/ipc';
import { isRemoteConsoleLog, log } from 'base/node/console';
	...
	private get client(): IPCClient {
		if (!this._client) {
			...
			this.child.on('exit', (code: number, signal: string) => {
				// hernad izglede ne treba vise sa nver electron
				// process.removeListener('disconnect', onExit);
