...
import { IWindowSettings, MenuBarVisibility, IWindowConfiguration, ReadyState, IRunActionInWindowRequest } from 'platform/windows/common/windows';
import { IDisposable, dispose } from 'base/common/lifecycle';
import { isLinux, isMacintosh, isWindows } from 'base/common/platform';
import { ICodeWindow, IWindowState, WindowMode } from 'platform/windows/electron-main/windows';
import { IWorkspaceIdentifier, IWorkspacesMainService } from 'platform/workspaces/common/workspaces';
import { IBackupMainService } from 'platform/backup/common/backup';
import { ISerializableCommandAction } from 'platform/actions/common/actions';
import * as perf from 'base/common/performance';
import { resolveMarketplaceHeaders } from 'platform/extensionManagement/node/extensionGalleryService';
...
export class CodeWindow implements ICodeWindow {
	...
	private registerListeners(): void {
		...
		this._win.webContents.on('did-fail-load', (event: Electron.Event, errorCode: number, errorDescription: string, validatedURL: string, isMainFrame: boolean) => {
			this.logService.warn(`[electron event]: fail to load: ${errorDescription} URL: ${validatedURL}`);
		});
	...
	private doGetUrl(config: object): string {
		return `${require.toUrl('code/electron-browser/workbench/workbench.html')}?config=${encodeURIComponent(JSON.stringify(config))}`;
	}
