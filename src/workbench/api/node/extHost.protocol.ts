...
import { CharacterPair, CommentRule, EnterAction } from 'editor/common/modes/languageConfiguration';
import { ICommandHandlerDescription } from 'platform/commands/common/commands';
import { ConfigurationTarget, IConfigurationData, IConfigurationModel } from 'platform/configuration/common/configuration';
import { ConfigurationScope } from 'platform/configuration/common/configurationRegistry';
import { FileChangeType, FileDeleteOptions, FileOverwriteOptions, FileSystemProviderCapabilities, FileType, FileWriteOptions, IStat, IWatchOptions } from 'platform/files/common/files';
import { LabelRules } from 'platform/label/common/label';
import { LogLevel } from 'platform/log/common/log';
import { IMarkerData } from 'platform/markers/common/markers';
import { IPickOptions, IQuickInputButton, IQuickPickItem } from 'platform/quickinput/common/quickInput';
import { IPatternInfo, IRawFileMatch2, IRawQuery, IRawTextQuery, ISearchCompleteStats } from 'platform/search/common/search';
import { StatusbarAlignment as MainThreadStatusBarAlignment } from 'platform/statusbar/common/statusbar';
import { ITelemetryInfo } from 'platform/telemetry/common/telemetry';
import { ThemeColor } from 'platform/theme/common/themeService';
import { EndOfLine, IFileOperationOptions, TextEditorLineNumbersStyle } from 'workbench/api/node/extHostTypes';
import { EditorViewColumn } from 'workbench/api/shared/editor';
import { TaskDTO, TaskExecutionDTO, TaskFilterDTO, TaskHandleDTO, TaskProcessEndedDTO, TaskProcessStartedDTO, /* TaskSystemInfoDTO hernad out-debug*/ } from 'workbench/api/shared/tasks';
import { ITreeItem, IRevealOptions } from 'workbench/common/views';
// import { IAdapterDescriptor, IConfig, ITerminalSettings } from 'workbench/parts/debug/common/debug'; extHost.protocol.ts
import { ITextQueryBuilderOptions } from 'workbench/parts/search/common/queryBuilder';
import { TaskSet } from 'workbench/parts/tasks/common/tasks';
import { ITerminalDimensions } from 'workbench/parts/terminal/common/terminal';
...
export interface MainThreadTaskShape extends IDisposable {
	...
	// $registerTaskSystem(scheme: string, info: TaskSystemInfoDTO): void; // hernad out-debug
}
...
export interface ExtHostTaskShape {
	...
	// $resolveVariables(workspaceFolder: UriComponents, toResolve: { process?: { name: string; cwd?: string }, variables: string[] }): Thenable<{ process?: string; variables: { [key: string]: string } }>;  hernad out-debug
}
...
export const MainContext = {
	...
	// MainThreadDebugService: createMainId<MainThreadDebugServiceShape>('MainThreadDebugService') ; hernad out-debug
...
export const ExtHostContext = {
	...
	// ExtHostDebugService: createExtId<ExtHostDebugServiceShape>('ExtHostDebugService'),  hernad out-debug
