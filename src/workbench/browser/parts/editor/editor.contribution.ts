...
import { IEditorQuickOpenEntry, IQuickOpenRegistry, Extensions as QuickOpenExtensions, QuickOpenHandlerDescriptor } from 'workbench/browser/quickopen';
import { StatusbarItemDescriptor, IStatusbarRegistry, Extensions as StatusExtensions } from 'workbench/browser/parts/statusbar/statusbar';
import { StatusbarAlignment } from 'platform/statusbar/common/statusbar';
import { IEditorRegistry, EditorDescriptor, Extensions as EditorExtensions } from 'workbench/browser/editor';
import { EditorInput, IEditorInputFactory, SideBySideEditorInput, IEditorInputFactoryRegistry, Extensions as EditorInputExtensions, TextCompareEditorActiveContext } from 'workbench/common/editor';
import { TextResourceEditor } from 'workbench/browser/parts/editor/textResourceEditor';
...
import { UntitledEditorInput } from 'workbench/common/editor/untitledEditorInput';
import { ResourceEditorInput } from 'workbench/common/editor/resourceEditorInput';
import { IInstantiationService } from 'platform/instantiation/common/instantiation';
import { TextDiffEditor } from 'workbench/browser/parts/editor/textDiffEditor';
import { ITextFileService } from 'workbench/services/textfile/common/textfiles';
import { BinaryResourceDiffEditor } from 'workbench/browser/parts/editor/binaryDiffEditor';
import { ChangeEncodingAction, ChangeEOLAction, ChangeModeAction, EditorStatus } from 'workbench/browser/parts/editor/editorStatus';
import { IWorkbenchActionRegistry, Extensions as ActionExtensions } from 'workbench/common/actions';
import { Scope, IActionBarRegistry, Extensions as ActionBarExtensions, ActionBarContributor } from 'workbench/browser/actions';
import { SyncActionDescriptor, MenuRegistry, MenuId, IMenuItem } from 'platform/actions/common/actions';
...
} from 'workbench/browser/parts/editor/editorActions';
import * as editorCommands from 'workbench/browser/parts/editor/editorCommands';
import { IEditorService } from 'workbench/services/editor/common/editorService';
import { getQuickNavigateHandler, inQuickOpenContext } from 'workbench/browser/parts/quickopen/quickopen';
...
import { AllEditorsPicker, ActiveEditorGroupPicker } from 'workbench/browser/parts/editor/editorPicker';
import { Schemas } from 'base/common/network';
import { registerEditorContribution } from 'editor/browser/editorExtensions';
import { OpenWorkspaceButtonContribution } from 'workbench/browser/parts/editor/editorWidgets';
...
if (isMacintosh) {
	MenuRegistry.appendMenuItem(MenuId.TouchBarContext, {
		command: { id: NavigateBackwardsAction.ID, title: NavigateBackwardsAction.LABEL, iconLocation: { dark: URI.parse(require.toUrl('workbench/browser/parts/editor/media/back-tb.png')) } },
		group: 'navigation'
	});

	MenuRegistry.appendMenuItem(MenuId.TouchBarContext, {
		command: { id: NavigateForwardAction.ID, title: NavigateForwardAction.LABEL, iconLocation: { dark: URI.parse(require.toUrl('workbench/browser/parts/editor/media/forward-tb.png')) } },
		group: 'navigation'
	});
}
...
function appendEditorToolItem(primary: IEditorToolItem, when: ContextKeyExpr, order: number, alternative?: IEditorToolItem): void {
	const item: IMenuItem = {
		command: {
			...
			title: primary.title,
			iconLocation: {
				dark: URI.parse(require.toUrl(`workbench/browser/parts/editor/media/${primary.iconDark}`)),
				light: URI.parse(require.toUrl(`workbench/browser/parts/editor/media/${primary.iconLight}`))
			}
		},
		group: 'navigation',
		when,
	...
	if (alternative) {
		item.alt = {
			...
			iconLocation: {
				dark: URI.parse(require.toUrl(`workbench/browser/parts/editor/media/${alternative.iconDark}`)),
				light: URI.parse(require.toUrl(`workbench/browser/parts/editor/media/${alternative.iconLight}`))
			}
		};
	}
