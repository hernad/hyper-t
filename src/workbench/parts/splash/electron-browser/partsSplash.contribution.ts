...
import { getTotalHeight, getTotalWidth } from 'base/browser/dom';
import { Color } from 'base/common/color';
import { anyEvent, debounceEvent } from 'base/common/event';
...
import { ColorIdentifier, editorBackground, foreground } from 'platform/theme/common/colorRegistry';
import { getThemeTypeSelector, IThemeService } from 'platform/theme/common/themeService';
import { DEFAULT_EDITOR_MIN_DIMENSIONS } from 'workbench/browser/parts/editor/editor';
import { Extensions, IWorkbenchContributionsRegistry } from 'workbench/common/contributions';
import * as themes from 'workbench/common/theme';
...
class PartsSplash {
	...
	private _savePartsSplash() {
		...
		if (baseTheme !== this._lastBaseTheme || colorInfo.editorBackground !== this._lastBackground) {
			...
			this.broadcastService.broadcast({ channel: 'hypert:changeColorTheme', payload: JSON.stringify({ baseTheme, background: Color.Format.CSS.formatHex(backgroundColor) }) });
		}
	}
