...
import { CommandOptions, Source, ErrorData } from 'base/common/processes';
import { LineProcess, LineData } from 'base/node/processes';
...
export class ProcessRunnerDetector {
	...
	private resolveCommandOptions(workspaceFolder: IWorkspaceFolder, options: CommandOptions): CommandOptions {
		// TODO@Dirk adopt new configuration resolver service https://github.com/hernad/hyper-t/issues/31365
	...
	private tryDetectGulp(workspaceFolder: IWorkspaceFolder, list: boolean): TPromise<DetectorResult> {
		return this.fileService.resolveFile(workspaceFolder.toResource('gulpfile.js')).then((stat) => { // TODO@Dirk (https://github.com/hernad/hyper-t/issues/29454)
	...
	private tryDetectGrunt(workspaceFolder: IWorkspaceFolder, list: boolean): TPromise<DetectorResult> {
		return this.fileService.resolveFile(workspaceFolder.toResource('Gruntfile.js')).then((stat) => { // TODO@Dirk (https://github.com/hernad/hyper-t/issues/29454)
	...
	private tryDetectJake(workspaceFolder: IWorkspaceFolder, list: boolean): TPromise<DetectorResult> {
		...
		return this.fileService.resolveFile(workspaceFolder.toResource('Jakefile')).then((stat) => { // TODO@Dirk (https://github.com/hernad/hyper-t/issues/29454)
			return run();
		}, (err: any) => {
			return this.fileService.resolveFile(workspaceFolder.toResource('Jakefile.js')).then((stat) => { // TODO@Dirk (https://github.com/hernad/hyper-t/issues/29454)
				return run();
			}, (err: any) => {
				return null;
			});
