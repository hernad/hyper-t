...
import { ITerminalConfiguration, getDefaultTerminalWindows, getDefaultTerminalLinuxReady, DEFAULT_TERMINAL_OSX } from 'workbench/parts/execution/electron-browser/terminal';
	...
	public runInTerminal(title: string, dir: string, args: string[], envVars: IProcessEnvironment): Promise<void> {
		...
		return new Promise<void>((c, e) => {
			...
			if (terminalApp === DEFAULT_TERMINAL_OSX || terminalApp === 'iTerm.app') {
				...
				const scriptpath = getPathFromAmdModule(require, `workbench/parts/execution/electron-browser/${script}.scpt`);
