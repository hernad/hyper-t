...
import { ILogService, LogLevel, DEFAULT_LOG_LEVEL } from 'platform/log/common/log';
...
export class SetLogLevelAction extends Action {
	...
	run(): Thenable<void> {
		...
		const entries = [
			...
			//{ label: nls.localize('debug', "Debug"), level: LogLevel.Debug, description: this.getDescription(LogLevel.Debug, current) },  hernad out-debug
