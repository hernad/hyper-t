...
import { ConfigureLocaleAction } from 'workbench/parts/localizations/electron-browser/localizationsActions';
import { ExtensionsRegistry } from 'workbench/services/extensions/common/extensionsRegistry';
...
import { IExtensionManagementService, DidInstallExtensionEvent, LocalExtensionType, IExtensionGalleryService, IGalleryExtension, InstallOperation } from 'platform/extensionManagement/common/extensionManagement';
...
import { VIEWLET_ID as EXTENSIONS_VIEWLET_ID, IExtensionsViewlet } from 'workbench/parts/extensions/common/extensions';
import { minimumTranslatedStrings } from 'platform/node/minimalTranslations';
				...
				this.galleryService.query({ text: `tag:lp-${locale}` }).then(tagResult => {
					...
					Promise.all([this.galleryService.getManifest(extensionToFetchTranslationsFrom, CancellationToken.None), this.galleryService.getCoreTranslation(extensionToFetchTranslationsFrom, locale)])
						.then(([manifest, translation]) => {
							...
							const translationsFromPack = translation && translation.contents ? translation.contents['platform/node/minimalTranslations'] : {};
