...
import { addClass, addDisposableListener } from 'base/browser/dom';
...
import { DARK, ITheme, IThemeService, LIGHT } from 'platform/theme/common/themeService';
import { registerFileProtocol, WebviewProtocol } from 'workbench/parts/webview/electron-browser/webviewProtocols';
...
export class WebviewElement extends Disposable {
	...
	dispose(): void {
		if (this._webview) {
			...
			// 2. guestinstance attribute (#13947)
			// With the OOPIF implementation, the guest WebContents is bound to a web frame forever, 
			// and it can not be detached and bound to another web frame. 
			// This is a design decision made from Chromium's side that we can not work around.
