...
import { QuickOpenModel, QuickOpenEntry, compareEntries } from 'base/parts/quickopen/browser/quickOpenModel';
import { IAutoFocus, Mode, IEntryRunContext } from 'base/parts/quickopen/common/quickOpen';
import * as filters from 'base/common/filters';
import * as strings from 'base/common/strings';
import { Range } from 'editor/common/core/range';
import { IWorkbenchEditorConfiguration } from 'workbench/common/editor';
...
import { IWorkspaceSymbolProvider, getWorkspaceSymbols, IWorkspaceSymbol } from 'workbench/parts/search/common/search';
		...
		TPromise.as(this.bearingResolve).then(() => {
			...
			if (scheme === Schemas.http || scheme === Schemas.https) {
				if (mode === Mode.OPEN || mode === Mode.OPEN_IN_BACKGROUND) {
					this.openerService.open(this.bearing.location.uri); // support http/https resources (https://github.com/hernad/hyper-t/issues/58924))
				}
			} else {
