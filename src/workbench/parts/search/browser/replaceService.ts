...
import { Match, FileMatch, FileMatchOrMatch, ISearchWorkbenchService } from 'workbench/parts/search/common/searchModel';
...
import { ITextModel, IIdentifiedSingleEditOperation } from 'editor/common/model';
import { IInstantiationService } from 'platform/instantiation/common/instantiation';
import { ResourceTextEdit } from 'editor/common/modes';
...
class ReplacePreviewModel extends Disposable {
	...
	resolve(replacePreviewUri: URI): Thenable<ITextModel> {
		...
		return this.textModelResolverService.createModelReference(fileResource).then(ref => {
			...
			this._register(fileMatch.onDispose(() => replacePreviewModel.dispose())); // TODO@Sandeep we should not dispose a model directly but rather the reference (depends on https://github.com/hernad/hyper-t/issues/17073)
