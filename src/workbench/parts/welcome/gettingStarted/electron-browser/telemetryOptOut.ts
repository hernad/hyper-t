...
import { IExperimentService, ExperimentState } from 'workbench/parts/experiments/node/experimentService';
		...
		if ((locale !== language && locale !== 'en' && locale.indexOf('en-') === -1)) {
			queryPromise = this.galleryService.query({ text: `tag:lp-${locale}` }).then(tagResult => {
				...
				return this.galleryService.getCoreTranslation(extensionToFetchTranslationsFrom, locale)
					.then(translation => {
						const translationsFromPack = translation && translation.contents ? translation.contents['workbench/parts/welcome/gettingStarted/electron-browser/telemetryOptOut'] : {};
