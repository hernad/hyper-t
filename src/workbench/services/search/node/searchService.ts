...
import { keys, ResourceMap, values } from 'base/common/map';
...
import { addContextToEditorMatches, editorMatchesToTextSearchResults } from 'workbench/services/search/common/searchHelpers';
...
export class DiskSearch implements ISearchResultProvider {
	...
	constructor(verboseLogging: boolean, timeout: number = 60 * 60 * 1000, searchDebug?: IDebugParams) {
		const opts: IIPCOptions = {
			...
			freshExecArgv: true,
			env: {
				AMD_ENTRYPOINT: 'workbench/services/search/node/searchApp',
				PIPE_LOGGING: 'true',
