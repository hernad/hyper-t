...
import { Uri, commands, Disposable, window, workspace, QuickPickItem, OutputChannel, Range, WorkspaceEdit, Position, LineChange, SourceControlResourceState, TextDocumentShowOptions, ViewColumn, ProgressLocation, TextEditor, MessageOptions, WorkspaceFolder } from 'hypert';
...
// import TelemetryReporter from 'hypert-extension-telemetry';
...
export class CommandCenter {
	...
	private async _openResource(resource: Resource, preview?: boolean, preserveFocus?: boolean, preserveSelection?: boolean): Promise<void> {
		...
		if (!left) {
			await commands.executeCommand<void>('hypert.open', right, opts, title);
		} else {
			await commands.executeCommand<void>('hypert.diff', left, right, title, opts);
		}
	}
	...
	async clone(url?: string): Promise<void> {
		...
		try {
			...
			*/
			...
			if (openFolder) {
				commands.executeCommand('hypert.openFolder', uri);
	...
	async openHEADFile(arg?: Resource | Uri): Promise<void> {
		...
		return await commands.executeCommand<void>('hypert.open', HEAD, opts);
	}
			...
			return result.catch(async err => {
				...
				switch (err.gitErrorCode) {
					...
					case GitErrorCodes.NoUserEmailConfigured:
						...
						choices.set(localize('learn more', "Learn More"), () => commands.executeCommand('hypert.open', Uri.parse('https://git-scm.com/book/en/v2/Getting-Started-First-Time-Git-Setup')));
						break;
					default:
