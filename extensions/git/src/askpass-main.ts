...
function main(argv: string[]): void {
	...
	if (!process.env['HYPERT_GIT_ASKPASS_HANDLE']) {
		return fatal('Missing handle');
	}

	if (!process.env['HYPERT_GIT_ASKPASS_PIPE']) {
		return fatal('Missing pipe');
	}

	if (process.env['HYPERT_GIT_COMMAND'] === 'fetch') {
		return fatal('Skip fetch commands');
	}

	const output = process.env['HYPERT_GIT_ASKPASS_PIPE'] as string;
	const socketPath = process.env['HYPERT_GIT_ASKPASS_HANDLE'] as string;
