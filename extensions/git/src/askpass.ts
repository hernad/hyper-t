...
import { Disposable, window, InputBoxOptions } from 'hypert';
...
export interface AskpassEnvironment {
	...
	HYPERT_GIT_ASKPASS_NODE?: string;
	HYPERT_GIT_ASKPASS_MAIN?: string;
	HYPERT_GIT_ASKPASS_HANDLE?: string;
}

function getIPCHandlePath(nonce: string): string {
	if (process.platform === 'win32') {
		return `\\\\.\\pipe\\hypert-git-askpass-${nonce}-sock`;
	}

	if (process.env['XDG_RUNTIME_DIR']) {
		return path.join(process.env['XDG_RUNTIME_DIR'] as string, `hypert-git-askpass-${nonce}.sock`);
	}

	return path.join(os.tmpdir(), `hypert-git-askpass-${nonce}.sock`);
}
...
export class Askpass implements Disposable {
	...
	async getEnv(): Promise<AskpassEnvironment> {
		...
		return {
			...
			HYPERT_GIT_ASKPASS_NODE: process.execPath,
			HYPERT_GIT_ASKPASS_MAIN: path.join(__dirname, 'askpass-main.js'),
			HYPERT_GIT_ASKPASS_HANDLE: await this.ipcHandlePathPromise
		};
	}
